...
- optional polygon hit outputs if city/lake vectors are configured

## 3) Prepare global city data

### GeoNames

Download a GeoNames file such as `cities500.zip`, `cities1000.zip`, `cities15000.zip`, or `allCountries.zip`, then run:

First, place the data somewhere:
```bash
mkdir -p data/raw
```


Smaller dataset fo cities:
```bash
...
Natural earth populated regions:
```bash
...
HydroLAKES global lakes polygons:
```bash
...
unzip -o data/raw/cities500.zip -d data/unpacked/cities500
...
unzip -o data/raw/ne_10m_land.zip -d data/unpacked/ne_10m_lan
```
...
python scripts/prepare_global_cities.py naturalearth \
  --input ne_10m_populated_places.shp \
  --out-base data/cities_global_ne \
  --min-scalerank 8 \
  --h3-res 7
```
